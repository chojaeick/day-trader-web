        ...
        for di in range(1,len(ds)):
            ...
            for wi in raw(dm[ds[di-1]],rows):
                ...
                for cfg in cfgs:
                    ...
                    trades[cfg].append((d,s,pct(closes[ei],closes[j])))
    print('=== WILLIAMS KOREA CAUSAL DAILY OOS V18 ===')
    print('CANDIDATES=W30/H5 and W60/H5; entry=first bar after rank<=20 confirmation')
    for cfg in cfgs:
        arr=trades[cfg]
        print(f'--- W{cfg[0]} H{cfg[1]} TOTAL_N={len(arr)} ---')
        by={}
        for d,s,r in arr: by.setdefault(d,[]).append((s,r))
        for d in sorted(by):
            vals=[r for _,r in by[d]]; m=metrics(vals)
            symlist=','.join(f'{s}:{r:.3f}' for s,r in by[d])
            print(f'{d} N={m["n"]} AVG={m["avg"]:.4f}% WIN={m["win"]:.2f}% PF={m["pf"]:.3f} | {symlist}')
        costs=(0.00,0.10,0.20,0.25)
        print('COST_STRESS_ALL')
        for c in costs:
            m=metrics([r-c for _,_,r in arr])
