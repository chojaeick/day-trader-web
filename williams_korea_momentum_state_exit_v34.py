...
def run_trade(rows,entry_i):
    ...
    entry=c[entry_i]; peak=entry; peak_ret=0; state='WATCH'; weak=0; confirm=0
    exit_i=len(rows)-1; reason='EOD'
    for i in range(entry_i+1,len(rows)):
        peak=max(peak,h[i]); peak_ret=max(peak_ret,pct(entry,peak))
        ret=pct(entry,c[i]); dd=pct(peak,c[i])
        r3=pct(c[max(entry_i,i-3)],c[i]) if i-entry_i>=3 else 0
        vnow=sum(v[max(entry_i,i-2):i+1]); vprev=sum(v[max(entry_i,i-5):max(entry_i,i-2)])
        vr=vnow/vprev if vprev>0 else 1

        strong = (r3>=0.3) + (c[i]>e5[i]>e10[i]) + (vr>=0.8)
        if strong>=2:
            confirm += 1
        else:
            confirm=max(0,confirm-1)

        if confirm>=2 and state=='WATCH':
            state='HOLD'
        if peak_ret>=3 and state in ('WATCH','HOLD'):
            state='RUNNER'
        ...
        # momentum loss requires persistence; runner gets more room
        below = c[i] < e5[i] and e5[i] < e10[i]
        if state=='RUNNER':
            below = below and c[i] < e20[i]
        if below:
            weak+=1
        else:
            weak=max(0,weak-1)
        ...
        if state=='WATCH':
            ...
            if weak>=4 and dd<=-1.2:
                exit_i=i; reason='HOLD_LOSS'; break
        else:
            # runner: let it breathe; profit zone widens with run-up
            trail = 1.8 if peak_ret<5 else 2.5 if peak_ret<10 else 3.5
            if weak>=5 and dd<=-trail:
                exit_i=i; reason='RUNNER_EXIT'; break

    mfe=max(pct(entry,x) for x in h[entry_i:])
    mae=min(pct(entry,x) for x in l[entry_i:])
    ret=pct(entry,c[exit_i])
    cap=(ret/mfe*100) if mfe>0 else 0
    return ret,mfe,mae,exit_i-entry_i,reason,cap

def metrics(arr):
    if not arr:return
    vals=[x[2] for x in arr]
    gp=sum(x for x in vals if x>0); gl=-sum(x for x in vals if x<0); pf=gp/gl if gl else 999
    print(f"N={len(arr)} AVG={statistics.fmean(vals):.3f}% WIN={100*sum(x>0 for x in vals)/len(vals):.1f}% PF={pf:.3f} MFE_AVG={statistics.fmean(x[3] for x in arr):.2f}% MAE_AVG={statistics.fmean(x[4] for x in arr):.2f}% HOLD_AVG={statistics.fmean(x[5] for x in arr):.1f}m CAPTURE_MED={statistics.median(x[7] for x in arr):.1f}%")
...
def main():
    ...
    for s in syms:
        ...
        for di in range(1,len(ds)):
            ...
            ei=idx+1
            ret,mfe,mae,hold,reason,cap=run_trade(cur,ei)
            trades.append((d,s,ret,mfe,mae,hold,reason,cap))
    print('=== WILLIAMS KOREA MOMENTUM STATE EXIT V34 ===')
    print('ENTRY=Williams next-bar. HOLD/EXIT uses momentum states, not fixed time.')
    metrics(trades)
    print('TRADES')
    for t in trades:
