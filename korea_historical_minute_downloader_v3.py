...
def main():
    ...
    body = {'stk_cd': symbol, 'tic_scope': str(args.interval), 'upd_stkpc_tp': '1', 'base_dt': target}
    cont_yn = None
    next_key = None
    collected = {}
    ...
    for page in range(1, args.max_pages + 1):
        ...
        print(f'PAGE {page} ROWS={len(rows)} TARGET_ROWS={page_target} TOTAL_TARGET={len(collected)} RANGE={min_date}..{max_date}')

        # Critical V3 guard: once this page is already older than target and target has not appeared,
        # later continuation pages can only be older still. Stop immediately instead of paging 30 times.
        if page_dates and max_date < target and not collected:
            print(f'NO_TRADING_DATA_OR_NOT_REACHED symbol={symbol} date={target} STOP_AT_PAGE={page}')
            break
        if collected and page_dates and min_date < target:
            break
    ...
    if not collected:
        print(f'KRX_MINUTE_NO_DATA SYMBOL={symbol} DATE={target}')
        return 2
