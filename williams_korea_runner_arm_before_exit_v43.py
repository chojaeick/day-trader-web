...
def run(rows,ei,mode):
    ...
    for i in range(ei+1,len(rows)):
        ...
        r1=pct(c[i-1],c[i]) if i-1>=ei else 0.0
        ...
        accel=(r1>0 and r3>0.3)
        # latent-runner evidence: enough expansion OR persistent positive continuation
        arm_now = (runup>=1.0) or (r3>=0.5 and ema_ok) or (r5>=0.8 and vr>=0.8) or (accel and vr>=0.8)
        runner_arm = runner_arm+1 if arm_now else max(0,runner_arm-1)
        ...
        if mode=='LEGACY':
            if state=='WATCH' and weak>=4 and below_struct and ret<0:
                exit_i=i; reason='WATCH_FAIL'; break
            if state=='HOLD' and weak>=4 and below_struct and dd<=-1.5:
                exit_i=i; reason='HOLD_FAIL'; break
        elif mode=='ARMED':
            # only weak-exit if not armed as latent runner
            if state=='WATCH' and runner_arm==0 and age>=5:
                if (ret<=-0.6) or (weak>=2 and below_struct) or (age>=10 and r5<=0 and ret<=0):
                    exit_i=i; reason='WEAK_EXIT'; break
            elif state=='HOLD' and runup<3 and age>=10:
                if weak>=3 and below_struct and dd<=-1.3 and runner_arm==0:
                    exit_i=i; reason='HOLD_FAIL'; break
        else: # ARMED_WIDE
            if state=='WATCH' and runner_arm==0 and age>=6:
                if (ret<=-0.8) or (weak>=3 and below_struct) or (age>=12 and r5<=0 and ret<=0):
                    exit_i=i; reason='WEAK_EXIT'; break
            elif state=='HOLD' and runup<3 and age>=14:
                if weak>=4 and below_struct and dd<=-1.5 and runner_arm==0:
                    exit_i=i; reason='HOLD_FAIL'; break
...
def main():
    ...
    print('=== WILLIAMS KOREA RUNNER ARM BEFORE EXIT V43 ===')
    print('Weak exits are blocked when latent-runner evidence is present. Runner logic remains V37 LOCK_TIGHT.')
    print('OOS_DATES',','.join(sorted(oos)))
    for mode in ('LEGACY','ARMED','ARMED_WIDE'):
        tr=[]
        ...
        for x in oo:
            if x[3]>=3:
                print(f"{x[0]} {x[1]} RET={x[2]:.2f}% MFE={x[3]:.2f}% HOLD={x[5]}m {x[6]} FINAL={x[8]}")
    con.close()
