...
def main():
    ...
    trades=build(con,args.max_days)
    m=metrics([r for _,_,r in trades]); m25=metrics([r-0.25 for _,_,r in trades])
    dates=sorted(set(d for d,_,_ in trades))
    pass_gate=(m['n']>=8 and len(dates)>=5 and m['avg']>0 and m['pf']>1.5 and m25['avg']>0 and m25['pf']>1.2)
    print('=== WILLIAMS KOREA CANDIDATE V22 ===')
    print('RULE=Williams CrossUp + RSI2>50 -> Finder rank<=20 within 30m -> enter next 1m bar -> hold 5m -> max one trade per symbol/day')
    print('DATES=',dates,'N=',m['n'])
    print(f'GROSS AVG={m["avg"]:.4f}% WIN={m["win"]:.2f}% PF={m["pf"]:.3f} MDD={m["mdd"]:.4f}%')
    print(f'COST25 AVG={m25["avg"]:.4f}% WIN={m25["win"]:.2f}% PF={m25["pf"]:.3f} MDD={m25["mdd"]:.4f}%')
    print('PASS_GATE=',pass_gate)
    print('STATUS=', 'VALIDATION_CANDIDATE' if pass_gate else 'HOLD')
    print('NOTE=Sample remains small; do not enable live/selectable until forward/OOS grows.')
    con.close()
