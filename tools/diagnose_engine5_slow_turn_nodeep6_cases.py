...
def main():
    ...
    # Reconstruct the same 96-candidate universe.
    parts = []
    ...
    out = sel.drop(columns=['event'], errors='ignore').merge(
        trades[['symbol','entry_time','exit_time','entry_price','exit_price','pnl_pct','net_pct','exit_reason']],
        on=['symbol','entry_time'], how='left', validate='one_to_one', suffixes=('','_sim')
    )
