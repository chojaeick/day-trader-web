...
def main():
    ...
    windows=(15,30,60); holds=(3,5,7,10)
    ...
    for s in syms:
        ...
        for di in range(1,len(ds)):
            ...
            for wi in raw(prev,rows):
                bt=parse_bar_ts(rows[wi][6])
                if not bt: continue
                for w in windows:
                    ev=first_confirm_after(evs,bt,w)
                    if not ev: continue
                    ei=first_bar_after(rows,ev[0])
                    if ei is None: continue
                    entry=closes[ei]
                    for h in holds:
                        j=min(ei+h,len(closes)-1)
                        rec[(w,h)].append((d,pct(entry,closes[j])))
                        dates[(w,h)].add(d)
    print('=== WILLIAMS KOREA CAUSAL EVENT HOLD OOS V16 ===')
    print('ENTRY=first 1m bar after first Finder rank<=20 confirmation; no future information')
    rowsout=[]
    for key in rec:
        ...
        rowsout.append((score,key,om))
    print('\n=== OOS RANK ===')
    for idx,(score,key,om) in enumerate(sorted(rowsout,reverse=True),1):
