...
def load_events(con):
    ...
    for ts,s,ev,st,power,rank in rows:
        ...
        if not dt: continue
        out.setdefault((str(s),dt.strftime('%Y%m%d')),[]).append((dt,float(power or 0),int(rank or 999999)))
    return out

def best_rank_event(events,bt,window=60):
    cand=[]
    for e in events:
        delta=abs((bt-e[0]).total_seconds())/60
        if delta<=window and e[2]<=20:
            cand.append((delta,e))
...
def main():
    ...
    con=sqlite3.connect(args.db); events=load_events(con)
    ...
    holds=(3,5,7,10,15,20)
    rec={h:[] for h in holds}; dates=set()
    for s in syms:
        ...
        for di in range(1,len(ds)):
            ...
            for i,closes,bts in raw(dm[ds[di-1]],dm[d]):
                bt=parse_bar_ts(bts)
                if not bt or not best_rank_event(evs,bt,60): continue
                dates.add(d)
                for h in holds:
    ...
    print('=== WILLIAMS KOREA RANK20 OOS HOLD GRID V12 ===')
    print('FILTER=Finder rank<=20 within 60m of raw Williams signal')
    print('DATES=',dts,'IS=',len(isd),'OOS=',len(oosd))
    rows=[]
    for h in holds:
        am=metrics([x for _,x in rec[h]]); im=metrics([x for d,x in rec[h] if d in isd]); om=metrics([x for d,x in rec[h] if d in oosd])
        score=om['avg']+min(om['pf'],3)*0.05+om['mdd']*0.002 if om['n'] else -999
        print(f"H{h} ALL N={am['n']} AVG={am['avg']:.4f}% PF={am['pf']:.3f} | IS N={im['n']} AVG={im['avg']:.4f}% PF={im['pf']:.3f} | OOS N={om['n']} AVG={om['avg']:.4f}% WIN={om['win']:.2f}% PF={om['pf']:.3f} MDD={om['mdd']:.4f}% SCORE={score:.4f}")
        rows.append((score,h,om))
    print('\n=== OOS RANK ===')
    for idx,(score,h,om) in enumerate(sorted(rows,reverse=True),1):
        print(f"{idx} H{h} OOS_N={om['n']} AVG={om['avg']:.4f}% WIN={om['win']:.2f}% PF={om['pf']:.3f} MDD={om['mdd']:.4f}% SCORE={score:.4f}")
    con.close()
