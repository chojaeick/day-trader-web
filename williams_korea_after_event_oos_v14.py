    ...
    for s in syms:
        ...
        for di in range(1,len(ds)):
            ...
            for i,closes,bts in raw(dm[ds[di-1]],dm[d]):
                ...
                if not bt: continue
                for w in windows:
                    if has_after(evs,bt,w):
                        for h in holds:
                            ret=pct(closes[i],closes[min(i+h,len(closes)-1)])
                            rec[(w,h)].append((d,ret)); dates[(w,h)].add(d)
    print('=== WILLIAMS KOREA AFTER-EVENT OOS V14 ===')
    print('FILTER=Finder rank<=20 event occurs AFTER Williams signal within window')
    for key in rec:
        ...
        print(f"W{key[0]} H{key[1]} DATES={dts} | ALL N={am['n']} AVG={am['avg']:.4f}% PF={am['pf']:.3f} | IS N={im['n']} AVG={im['avg']:.4f}% PF={im['pf']:.3f} | OOS N={om['n']} AVG={om['avg']:.4f}% WIN={om['win']:.2f}% PF={om['pf']:.3f} MDD={om['mdd']:.4f}%")
    con.close()
