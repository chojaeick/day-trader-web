...
def main():
    ...
    s=s.replace(
        ...
        "rows=con.execute("select et_time,open,high,low,close,volume from historical_minute_bars where symbol=? and trade_date=? and session='REGULAR' order by et_time",(symbol,d)).fetchall()",
        1
    )

    old="""            # Robust ET time parsing.
...
"""
    ...
    new="""            # Use explicit ET time from historical_minute_bars.et_time.
            et=str(ts).strip()
            hhmm=None
            try:
                if 'T' in et:
...
"""
    ...
    if old not in s:
        raise SystemExit('PATCH_TARGET_NOT_FOUND: V2.1 parser block')
