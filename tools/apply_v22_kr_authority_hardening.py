...
def patch_api():
    ...
    # Disable the legacy Williams watchdog robustly regardless of how its task is formatted.
    import re
    before=s
    s=re.sub(r'\s*asyncio\.create_task\(williams_mock_hard_stop_forever\(\)\),?','',s)
    if s!=before:
...
'''
    ...
    # Inject the EOD task immediately before the v4_engine task, independent of surrounding formatting.
    if 'asyncio.create_task(v22_kr_eod_flatten_forever())' not in s:
        ...
        for p in patterns:
            if p in s:
                repl='asyncio.create_task(v22_kr_eod_flatten_forever()),\n                      '+p
                s=s.replace(p,repl,1)
                done=True
                break
        if not done:
            raise SystemExit('ABORT lifespan v4 task anchor missing')
...
def main():
    ...
    if not Path(str(V4)+'.pre_v22_authority_hardening').exists():
        run('sudo','cp','-p',V4,str(V4)+'.pre_v22_authority_hardening')
    if not Path(str(API)+'.pre_v22_authority_hardening').exists():
