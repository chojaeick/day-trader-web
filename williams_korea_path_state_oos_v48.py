...
def run(rows,ei,mode):
    ...
    for i in range(ei+1,len(rows)):
        ...
        m10=maxk(10); m20=maxk(20); n10=mink(10)

        # runner evidence can appear early; weak state is deliberately delayed
        if age>=5 and (m10>=2.0 or r5>=1.0 or (age>=10 and r10>=1.0)):
            state='FAST_RUNNER'
        elif age>=15 and ((m20>=2.0 and (r15>=0.5 or r20>=0.8)) or (maxk(15)>=1.5 and r15>0)):
            state='LATENT_RUNNER'
        ...
        if state=='WEAK':
            if mode=='DELAYED_WEAK':
                if ret<=-0.8 or (weakbars>=3 and below_struct):
                    exit_i=i; reason='WEAK_EXIT'; break
            else:
                if ret<=-1.0 or (weakbars>=4 and below_struct):
                    exit_i=i; reason='WEAK_EXIT'; break
            continue

        # uncertain gets time; only clear structural failure can exit
        if age>=20 and weakbars>=5 and below_struct and ret<0:
...
def main():
    ...
    print('=== WILLIAMS KOREA PATH STATE OOS V48 ===')
    print('Weak classification delayed to 20m; runner evidence can promote earlier. No fixed holding time.')
    print('OOS_DATES',','.join(sorted(oos)))
    for mode in ('DELAYED_WEAK','DELAYED_WEAK_SOFT'):
        tr=[]
