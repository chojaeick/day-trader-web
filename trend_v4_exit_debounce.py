...
# Finder/SCORE unchanged from V1-V3.
def score_row(x, i):
    if i < 6:
        return 0, {}
    r = x.loc[i]
...
def position_return(legs, px, deployed):
    return weighted_return(legs, px) / deployed if deployed > 0 else 0.0
...
def trade_one(x, i0):
    ...
    for i in range(i0+1, min(i0+MAX_HOLD, len(x)-1)+1):
        ...
        cur_posret = position_return(legs, sig, deployed)
        peak_posret = max(peak_posret, cur_posret)

        # Profit capture on deployed-capital return.
        floor = None
        if peak_posret >= 3.0:
            floor = 1.80
        ...
        # Absolute risk kill switch: only after confirmed loss, to avoid
        # letting a false trend drift indefinitely.
        if cur_posret <= -1.20:
            exit_px = px
            exit_i = i
        ...
        # V4 structural exit: require persistence, not one noisy bar.
        ...
        if not confirm_done:
            ...
            if confirm_ok:
                ...
                deployed += CONFIRM_W
                confirm_done = True
                peak_posret = max(0.0, position_return(legs, sig, deployed))

        # Delayed FULL logic from V2/V3.
        ...
        # V4 full-position failure also requires persistence.
        if full_done:
    ...
    net = gross - cost
    final_posret = position_return(legs, exit_px, deployed)
...
def main():
    ...
    print("HARD_RISK_EXITS", int((z.EXIT_REASON == "HARD_RISK").sum()))
    ...
    cols = [
        ...
        "EXIT_REASON","GROSS","COST","NET",
        "PEAK_POSRET","FLOOR_POSRET","FINAL_POSRET","MFE30","MAE30"
    ]
