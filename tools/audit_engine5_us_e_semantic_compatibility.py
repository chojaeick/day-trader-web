...
def clip_entry(ev): return {pd.Timestamp(ts):list(cs) for ts,cs in ev.items() if US_BUY<=minute(ts)<US_NO_ENTRY}
...
def audit_cache(rows,d):
    ...
    day_rows=[]; bad_ts=bad_rows=bad5=dup=0
    for s,b in raw.items():
        z=b.copy(); z['time']=pd.to_datetime(z.time); z['day']=z.time.dt.date; dup+=int(z.time.duplicated().sum())
        for day,g in z.groupby('day'):
            ...
            if len(f5)!=78 or minute(f5.time.iloc[0])!=9*60+35 or minute(f5.time.iloc[-1])!=16*60: bad5+=1
...
def audit_scale_semantics(rows,d):
    s=sorted(d['raw'])[0]; b=d['raw'][s].copy(); b['time']=pd.to_datetime(b.time); b=b.iloc[:390*5].copy()
    f5=to_5m(b); b10=b.copy()
    for c in ['open','high','low','close']: b10[c]=pd.to_numeric(b10[c],errors='coerce')*10.0
    eng=DoubleBollingerEngine5(); a=eng.enrich(f5); z=eng.enrich(to_5m(b10)); common=set(a.columns)&set(z.columns)
    inv=[c for c in common if any(k in c.lower() for k in ['rsi','ratio','strength'])]; bad=[]
    for c in inv:
        aa=pd.to_numeric(a[c],errors='coerce'); zz=pd.to_numeric(z[c],errors='coerce'); mask=aa.notna()&zz.notna()
        if mask.any() and not np.allclose(aa[mask],zz[mask],rtol=1e-9,atol=1e-9): bad.append(c)
    ok(rows,'scale_invariant_indicators',not bad,'RSI/ratio/strength invariant under x10 price scale','mismatch: '+','.join(bad[:12]))
    ...
    bb=[]
    for c in bool_cols:
        m=int((a[c].fillna(False).astype(bool).to_numpy()!=z[c].fillna(False).astype(bool).to_numpy()).sum())
        if m: bb.append((c,m))
    total=sum(m for _,m in bb); add(rows,'scale_boolean_gates','PASS' if total==0 else ('WARN' if total<=10 else 'FAIL'),'identical' if total==0 else 'mismatches='+','.join(f'{c}:{m}' for c,m in bb[:12]))


def audit_code_leaks(rows):
    src=inspect.getsource(e); bad=[]
    ...
    need=['US_BUY_START_MINUTE','US_OPENING_END_MINUTE','US_NO_ENTRY_MINUTE','US_FORCE_FLAT_MINUTE','clip_us_entry_window']
    missing=[x for x in need if x not in src]
    ok(rows,'e_session_adapter',not missing,'US session adapter + explicit entry clip','missing '+','.join(missing))
...
def audit_event_semantics(rows,d):
    apply_us_clock(); raw=d['raw']; cfg=d['cfg']; scored=d['scored']; micros=d['micros']
    raw_entries=v8.pack_entry_events(scored)
    ev10=clip_entry(sweep.filt_open(raw_entries))
    ev16,waits=v16.build_wait_events(ev10,raw,cfg,False); ev16=clip_entry(ev16)
    ev17,_,_=v17b.build_v17b(ev16,scored,waits); ev17=clip_entry(ev17)
    ev18,_=h.build_veto_stream(ev17,micros); ev18=clip_entry(ev18)
    inside=lambda ev: all(US_BUY<=minute(ts)<US_NO_ENTRY for ts in ev)
    ...
    fast,_=v19.build_v19_events(scored,micros,raw,0); fast=clip_entry(fast)
    merged,_=v19.merge_additive(ev18,fast); merged=clip_entry(merged)
...
def main():
    ...
    rows=[]
    print('=== ENGINE5 US E-SERIES SEMANTIC COMPATIBILITY AUDIT ==='); print('NO PERFORMANCE / NO WIN RATE / NO PNL.')
    audit_cache(rows,d); audit_session_mapping(rows); audit_scale_semantics(rows,d); audit_code_leaks(rows); audit_event_semantics(rows,d)
    out=pd.DataFrame(rows); out.to_csv(OUT,index=False); print('\n'+out.to_string(index=False))
    fails=int((out.status=='FAIL').sum()); warns=int((out.status=='WARN').sum()); print('\n=== VERDICT ===')
    if fails:
        print(f'FAIL: semantic applicability NOT proven. fails={fails} warns={warns}'); print('Do not interpret performance until all semantic FAIL items are resolved.')
    else:
        print('PASS'+(f' WITH WARNINGS({warns})' if warns else '')+': E-series implementation preserves KR Engine5 semantics on native USD/original ET at audited layers.')
