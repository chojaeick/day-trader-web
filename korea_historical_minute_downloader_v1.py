...
def ensure_table(con):
    ...
    CREATE TABLE IF NOT EXISTS historical_minute_bars (
      ...
      source TEXT,
      fetched_at TEXT,
      PRIMARY KEY(symbol, trade_date, interval_min, ts)
    )
    ''')
...
def main():
    ...
    for page in range(1, args.max_pages + 1):
        ...
        for x in rows:
            ...
            # Keep DB compatibility: ts is an offset-aware ISO timestamp; et_time stores clock text.
    ...
    try:
        ...
        '''
        con.executemany(sql, [collected[k] for k in sorted(collected)])
        con.commit()
        n = con.execute('SELECT COUNT(*) FROM historical_minute_bars WHERE symbol=? AND trade_date=? AND interval_min=?',
                        (symbol, target, args.interval)).fetchone()[0]
    finally:
        con.close()

    print(f'KRX_MINUTE_DOWNLOAD_OK SYMBOL={symbol} DATE={target} SAVED_ROWS={n}')
