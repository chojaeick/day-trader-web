...
def run(rows,ei,mode):
    ...
    for i in range(ei+1,len(rows)):
        ...
        if state=='WATCH':
            ...
            # compare profit protection for runners
            if mode=='BASE':
                trail=2.0 if runup<5 else 3.0 if runup<10 else 4.0
                need=5
            elif mode=='LOCK_STEP':
                trail=1.6 if runup<5 else 2.2 if runup<10 else 3.0
                need=5
            else: # LOCK_TIGHT
                trail=1.3 if runup<5 else 1.8 if runup<10 else 2.5
                need=4
            # do not exit on price wiggle alone: require structural loss too
            if weak>=need and below_struct and dd<=-trail:
                exit_i=i; reason='RUNNER_STRUCT_EXIT'; break
            # profit floor only after real run-up, still causal
            if mode in ('LOCK_STEP','LOCK_TIGHT'):
                floor = None
                if runup>=10: floor=6.0 if mode=='LOCK_STEP' else 7.0
                elif runup>=7: floor=4.0 if mode=='LOCK_STEP' else 4.8
                elif runup>=5: floor=2.5 if mode=='LOCK_STEP' else 3.2
                elif runup>=3: floor=1.2 if mode=='LOCK_STEP' else 1.6
...
def metrics(name,tr):
    ...
    bigcap=statistics.fmean(x[7] for x in big) if big else 0
    print(f"{name} N={len(tr)} AVG={statistics.fmean(vals):.3f}% WIN={100*sum(x>0 for x in vals)/len(vals):.1f}% PF={pf:.3f} MDD={mdd:.3f}% HOLD_AVG={statistics.fmean(x[5] for x in tr):.1f}m CAP_MED={statistics.median(x[7] for x in tr):.1f}% BIG5_CAP_AVG={bigcap:.1f}%")
    print('BIG_MFE>=5')
    for t in big:
...
def main():
    ...
    print('=== WILLIAMS KOREA STRUCTURAL RUNNER CAPTURE V37 ===')
    print('ENTRY=Williams next-bar. No fixed holding time. Compare runner profit capture only.')
    for mode in ('BASE','LOCK_STEP','LOCK_TIGHT'):
        tr=[]
