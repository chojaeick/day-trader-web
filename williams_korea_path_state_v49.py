...
def run(rows,ei,mode):
    ...
    for i in range(ei+1,len(rows)):
        age=i-ei; peak=max(peak,h[i]); runup=pct(entry,peak); ret=pct(entry,c[i]); dd=pct(peak,c[i])
        def retk(k):
            j=min(ei+k,i); return pct(entry,c[j])
        def maxk(k):
            j=min(ei+k,i); return max(pct(entry,x) for x in h[ei:j+1])
        def mink(k):
            j=min(ei+k,i); return min(pct(entry,x) for x in l[ei:j+1])
        r5=retk(5) if age>=5 else ret
        r10=retk(10) if age>=10 else ret
        r15=retk(15) if age>=15 else ret
        r20=retk(20) if age>=20 else ret
        m10=maxk(10); m15=maxk(15); m20=maxk(20); n10=mink(10)

        # runner evidence can appear early and always has priority over weak state
        if runup>=3 or (age>=5 and (m10>=2.0 or r5>=1.0 or (age>=10 and r10>=1.0))):
            state='FAST_RUNNER'
        elif age>=15 and ((m20>=2.0 and (r15>=0.5 or r20>=0.8)) or (m15>=1.5 and r15>0)):
            state='LATENT_RUNNER'
        elif age>=20 and m20<0.8 and r10<0 and r20<0 and n10<=-0.5:
            state='WEAK'
        else:
            state='UNCERTAIN'
        ...
        if state in ('FAST_RUNNER','LATENT_RUNNER'):
            ...
            # latent runners below 3% should not be killed by a single swing
            need=4 if runup>=3 else 6
            reqdd=trail if runup>=3 else 1.8
            if weakbars>=need and below_struct and dd<=-reqdd:
                exit_i=i; reason='RUNNER_STRUCT_EXIT'; break
            continue

        if state=='WEAK':
            # only exit after 20m of confirmed weak path
            limit=-0.8 if mode=='BALANCED' else -1.0
            need=3 if mode=='BALANCED' else 4
            if ret<=limit or (weakbars>=need and below_struct):
                exit_i=i; reason='WEAK_EXIT'; break
            continue

        # uncertain gets more time; no early percent stop
        if age>=25 and weakbars>=5 and below_struct and ret<0:
...
def main():
    ...
    print('=== WILLIAMS KOREA PATH STATE V49 ===')
    print('Runner evidence has priority; WEAK only after 20m confirmed failure; UNCERTAIN gets time.')
    print('OOS_DATES',','.join(sorted(oos)))
    for mode in ('BALANCED','SOFT'):
        tr=[]
        ...
        for x in oo:
            if x[3]>=3:
                print(f"{x[0]} {x[1]} RET={x[2]:.2f}% MFE={x[3]:.2f}% HOLD={x[5]}m {x[6]} LAST={x[8]}")
    con.close()
