...
def render_buy_box(r,market):
    ...
    with st.expander(f'매수 계산 / 상세 · {symbol}'):
        ...
        st.warning('수동 주문 전용입니다. 이 화면은 주문을 전송하지 않습니다.')
...
def render_positions(market):
    ...
    for p in pos_rows or []:
        ...
        shown+=1; sym=p.get('symbol') or '-'; avg,qty,cur,pnl,pct,floor,ceiling,t1,t2=position_values(p,market)
        ...
        st.markdown(f'''<div class="v5-card"><div class="v5-kicker">POSITION INTELLIGENCE</div><div><b>Floor</b> {money(floor,market) if floor is not None else '-'} · <b>Ceiling</b> {money(ceiling,market) if ceiling is not None else '-'} · <b>T1</b> {money(t1,market) if t1 is not None else '-'} · <b>T2</b> {money(t2,market) if t2 is not None else '-'}</div><div class="v5-muted">손절/익절 판단은 기존 엔진 값을 표시하며 실제 매도는 수동입니다.</div></div>''',unsafe_allow_html=True)
    if not shown: st.caption('등록된 포지션이 없거나 포지션 API 연결을 확인해야 합니다.')
...
def render_trading(market):
    ...
    c1,c2,c3,c4=st.columns(4); c1.metric('시장',market); c2.metric('세션',session); c3.metric('추천 후보',len(finders)); c4.metric('실시간 관리',len(rows))
    st.subheader('⚡ 지금 단타 후보'); st.caption('최종 행동 판단 → 매수금액 계산 → 수동 주문 순서로 사용합니다.')
    source=rows if rows else finders
    if source:
        st.dataframe(recommendation_table(source,market),use_container_width=True,hide_index=True)
        for r in source[:3]:
            symbol=r.get('symbol') or '-'; name=r.get('name') or symbol; act=action_of(r); price=r.get('price') or r.get('current_price'); power=f(r.get('power')); reason=r.get('prototype_reason') or r.get('reason') or r.get('core_reason') or '엔진 판단 근거 대기'
    ...
    render_positions(market)
    st.subheader('📈 중장기 후보'); st.info('단타와 분리된 장기 엔진/월봉·기본정보 연결 단계입니다.')
