...
def ensure_midline(data):
    for sym,x in data.items():
        # Rebuild the completed-5m Bollinger midline causally and map to 1m rows.
        ...
        b5['bb_mid']=b5['close'].rolling(20).mean()
        b5['bb_mid_prev']=b5['bb_mid'].shift(1)
        b5['close5_prev_for_mid']=b5['close'].shift(1)
        t=pd.to_datetime(x['et'])
        x['_mid_bucket']=t.dt.floor('5min')-pd.Timedelta(minutes=5)
        m=b5[['bucket_raw','bb_mid','bb_mid_prev','close5_prev_for_mid']].rename(columns={'bucket_raw':'_mid_bucket'})
        data[sym]=x.merge(m,on='_mid_bucket',how='left').drop(columns=['_mid_bucket'])
    return data
...
def add_trade(trades,sym,entry,partial_price,final_price,partial_done,reason,cost_bps,mfe,mae):
    ...
    if partial_done:
        # 50% round trip at partial and 50% round trip at final exit.
        gross=0.5*(partial_price/entry-1.0)+0.5*(final_price/entry-1.0)
        net=gross-2.0*fee
    else:
        gross=final_price/entry-1.0
        net=gross-2.0*fee
    trades.append({
        'engine':'DBB_ORIGINAL_ENTRY_2R_HALF_MIDLINE', 'symbol':sym,
        'entry_price':entry,'partial_price':partial_price if partial_done else None,
...
def metrics(trades):
    ...
    r=pd.Series([t['net_return'] for t in trades],dtype=float)
    wins=int((r>0).sum()); gp=float(r[r>0].sum()); gl=float(-r[r<0].sum())
    eq=(1.0+r).cumprod(); peak=eq.cummax(); dd=eq/peak-1.0
    return {
        'trades':len(trades),'wins':wins,'win_rate':wins/len(trades),
        ...
        'avg_mae_pct':float(pd.Series([t['mae'] for t in trades]).mean()*100),
    ...
    for t in all_times:
        if pos:
            ...
            if t in maps[sym]:
                ...
                p=float(x.at[i,'close']); hi=float(x.at[i,'high']); lo=float(x.at[i,'low'])
                mfe=max(mfe,hi/entry-1.0); mae=min(mae,lo/entry-1.0)

                # Safety stop always remains valid.
                if lo <= stop:
                    add_trade(trades,sym,entry,partial_price,stop,partial_done,'INITIAL_STOP',cost_bps,mfe,mae)
                    pos=None; continue

                # First profit action: exactly 2R from the frozen initial risk, sell 50% only.
                if (not partial_done) and hi >= target:
                    partial_done=True
                    partial_price=target

                # Runner: after 2R partial, keep holding while trend remains above the 20-period DBB midline.
                # Exit remaining 50% only on a completed 5m close crossing downward through the midline.
                if partial_done:
                    mid=x.at[i,'bb_mid']; mid_prev=x.at[i,'bb_mid_prev']; close5=x.at[i,'close5']; close5_prev=x.at[i,'close5_prev_for_mid']
                    if all(pd.notna(v) for v in (mid,mid_prev,close5,close5_prev)):
                        mid_cross_down=float(close5_prev) >= float(mid_prev) and float(close5) < float(mid)
                        if mid_cross_down:
                            add_trade(trades,sym,entry,partial_price,p,True,'MIDLINE_CROSS_DOWN',cost_bps,mfe,mae)
                            pos=None; continue

                et=pd.Timestamp(x.at[i,'et'])
                if et.hour==15 and et.minute>=59:
                    add_trade(trades,sym,entry,partial_price,p,partial_done,'SESSION_CLOSE',cost_bps,mfe,mae)
                    pos=None; continue
...
def main():
    ...
    data=ensure_midline(data)
    print(f'V247 SOURCE={table} DAYS={len(dates)} BARS={len(bars)} ENTRY=V240D_ORIGINAL',flush=True)
    tr=replay(data,a.fallback_risk_pct,a.max_swing_risk_pct,a.cost_bps)
    m=metrics(tr)
    print('V247_METRICS=',json.dumps(m),flush=True)
    if tr:
        df=pd.DataFrame(tr)
        print('PARTIAL_2R_COUNT=',int(df['partial_done'].sum()),flush=True)
        print('EXIT_REASONS=',json.dumps(df['exit_reason'].value_counts().to_dict()),flush=True)
        print('SYMBOLS=',json.dumps(df['symbol'].value_counts().to_dict()),flush=True)
        print('MFE_GE_2P=',int((df['mfe']>=0.02).sum()),flush=True)
        print('PARTIAL_THEN_LOSS=',int(((df['partial_done']==True)&(df['net_return']<0)).sum()),flush=True)
