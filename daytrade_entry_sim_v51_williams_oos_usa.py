...
MODES=['MACD_CCI_COMBO','COMBO_2BAR']
...
def find_entry(prev,cur):
    ...
    for i in range(20,len(cur)):
        ...
        return {'i':i,'entry':closes[i],'cci':c20,'macd':macd,'sig':sig,'hist':hist,'highs':highs,'lows':lows,'closes':closes,'rows':cur}
    return None
...
def show(label,mode,z):
    if not z:
        print(label,mode,'N=0'); return
    print(label,mode,'N=',z['n'],'AVG=',f"{z['avg']:.4f}%",'WIN=',f"{z['win']:.2f}%",'PF=',f"{z['pf']:.3f}",'MDD=',f"{z['mdd']:.4f}%",'AVG_HOLD=',f"{z['avg_hold']:.1f}m",'MED_HOLD=',f"{z['med_hold']:.1f}m",'MFE_CAPTURE=',('NA' if z['capture'] is None else f"{z['capture']:.1f}%"))
...
def main():
    ...
    for s in syms:
        ...
        for di in range(1,len(ds)):
            ...
            if not e: continue
            for m in MODES:
                ix=exit_index(e,m); entry=e['entry']; xp=e['closes'][ix]; ret=pct(entry,xp); hold=ix-e['i']
                mfe=max(0.0,pct(entry,max(e['highs'][e['i']:ix+1])))
                # Profit-capture efficiency: 0 for losing/flat exits, capped at 100 for profitable exits.
                capture=(min(100.0,max(0.0,ret)/mfe*100.0) if mfe>0 else None)
                raw.append({'symbol':s,'date':str(ds[di]),'mode':m,'ret':ret,'hold':hold,'capture':capture})
        print('AUDIT',s,'DAYS=',max(0,len(ds)-1),'ENTRIES=',sum(1 for x in raw if x['symbol']==s and x['mode']==MODES[0]))
    con.close()

    dates=sorted(set(x['date'] for x in raw)); split=len(dates)//2; is_dates=set(dates[:split]); oos_dates=set(dates[split:])
    print('\n=== WILLIAMS V5.1 OOS EXIT VALIDATION USA ===')
    print('ENTRY=FIRST + 09:30-11:00 + volume>=1.5x prior10 + RSI2>50 + CCI20>100 + MACD hist rising')
    ...
    for m in MODES:
        allz=metrics([x for x in raw if x['mode']==m]); isz=metrics([x for x in raw if x['mode']==m and x['date'] in is_dates]); oosz=metrics([x for x in raw if x['mode']==m and x['date'] in oos_dates])
        show('ALL',m,allz); show('IS ',m,isz); show('OOS',m,oosz)
        if oosz:
            pass_flag = bool(oosz['avg']>0 and oosz['pf']>1.2 and oosz['n']>=15)
            print('OOS_PASS',m,'=',pass_flag)
