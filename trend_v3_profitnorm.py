...
# Finder/SCORE unchanged from TREND v1/v2.
def score_row(x, i):
    if i < 6:
        return 0, {}
    r = x.loc[i]
...
def position_return(legs, px, deployed):
    # Normalize weighted account P&L by deployed capital.
    # This is the correct scale for a +1%, +2% profit-state rule.
    if deployed <= 0:
        return 0.0
    return weighted_return(legs, px) / deployed
...
def trade_one(x, i0):
    ...
    for i in range(i0+1, min(i0+MAX_HOLD, len(x)-1)+1):
        ...
        # Use completed signal price to update the position-level profit state.
        cur_posret = position_return(legs, sig, deployed)
        peak_posret = max(peak_posret, cur_posret)

        # Profit floor is defined on RETURN OF DEPLOYED CAPITAL, not whole-account P&L.
        floor = None
        if peak_posret >= 3.0:
            floor = 1.80
        ...
        # Same delayed FULL logic as v2: breakout must persist or retest successfully.
...
def main():
    ...
    print("===== TREND V3 NORMALIZED PROFIT FLOOR =====")
