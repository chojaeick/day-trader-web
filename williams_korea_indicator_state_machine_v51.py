...
def run(rows,ei,mode):
    ...
    entry=c[ei]; peak=entry; state='WATCH'; weak_streak=0; exit_i=len(rows)-1; reason='EOD'

    for i in range(max(ei+1,15),len(rows)):
        peak=max(peak,h[i]); ret=pct(entry,c[i]); runup=pct(entry,peak)
        # current slopes / crosses
        r_now=r14[i] if r14[i] is not None else 50
        r_prev=r14[i-1] if r14[i-1] is not None else r_now
        c_now=cc[i] if cc[i] is not None else 0
        c_prev=cc[i-1] if cc[i-1] is not None else c_now
        r_down = r_now < r_prev
        c_down = c_now < c_prev
        mac_down = mac[i] < mac[i-1]
        sig_down = sig[i] < sig[i-1]
        ...
        rebound = (r_prev <= 30 and r_now > r_prev) or golden or (hist[i] > hist[i-1] and hist[i-1] <= hist[i-2])

        strong_axes = sum([
            r_now >= 60 and not r_down,
            c_now >= 100 and not c_down,
            mac[i] > sig[i] and not mac_down,
            hist[i] >= hist[i-1]
        ])
        weak_axes = sum([
            r_down,
        ...
        # Big-runner exit conditions
        if state=='RUNNER':
            if mode=='OR':
                fire = rsi75_exit or cci100_exit or dead
            elif mode=='AND2':
                fire = sum([rsi75_exit,cci100_exit,dead,mac_down and sig_down])>=2
            else: # HYBRID
                fire = (rsi75_exit and cci100_exit) or (dead and (r_down or c_down)) or (weak_axes>=3)
            if fire and ret>0:
                exit_i=i; reason='RUNNER_IND_EXIT'; break
            continue

        # Non-runner: weak momentum should be cut, but REARM remains watchable
        if state=='REARM':
        ...
        if weak_streak>=threshold and ret<=0:
            exit_i=i; reason='WEAK_MOMENTUM_EXIT'; break
...
def main():
    ...
    print('=== WILLIAMS KOREA INDICATOR STATE MACHINE V51 ===')
    print('Entry fixed. RSI14/CCI9/MACD define WATCH/STRONG/RUNNER/REARM and exits.')
    print('OOS_DATES',','.join(sorted(oos)))
    for mode in ('OR','AND2','HYBRID'):
        tr=[]
