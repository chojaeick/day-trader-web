...
def run(rows,ei,mode):
    ...
    for i in range(ei+1,len(rows)):
        ...
        # Runner promotion is sacred: actual expansion >=3% always wins.
        if runup>=3:
            state='RUNNER'
        elif runup>=1:
            state='HOLD'
        ...
        # Key redesign: weak exit only when the trade has NOT shown real expansion.
        if mode=='LEGACY':
        ...
        elif mode=='EARLY_WEAK':
            # protect candidates that have shown 1%+ expansion; only kill clearly non-expanding trades
            if runup<1.0 and age>=5:
                if (ret<=-0.55) or (weak>=2 and below_struct) or (age>=10 and r5<=0 and ret<=0):
                    exit_i=i; reason='WEAK_NONEXPAND_EXIT'; break
            elif state=='HOLD' and runup<3 and age>=12:
                if weak>=3 and below_struct and dd<=-1.2:
                    exit_i=i; reason='HOLD_FAIL'; break
        else: # EARLY_WEAK_SOFT
            if runup<0.8 and age>=6:
                if (ret<=-0.70) or (weak>=3 and below_struct) or (age>=12 and r5<=0 and ret<=0):
                    exit_i=i; reason='WEAK_NONEXPAND_EXIT'; break
            elif state=='HOLD' and runup<3 and age>=15:
                if weak>=4 and below_struct and dd<=-1.4:
                    exit_i=i; reason='HOLD_FAIL'; break
...
def main():
    ...
    print('=== WILLIAMS KOREA RUNNER PRESERVE / WEAK EXIT V42 ===')
    print('Principle: never sacrifice actual 3%+ expansion to save weak trades. Tighten only non-expanding signals.')
    print('OOS_DATES',','.join(sorted(oos)))
    for mode in ('LEGACY','EARLY_WEAK','EARLY_WEAK_SOFT'):
        tr=[]
