...
class DoubleBollingerConfig:
    ...
    swing_left: int = 2
    swing_right: int = 2
    # Optional policy. None means the core does NOT force a fixed profit target.
    profit_check_pct: Optional[float] = None
    normal_partial_fraction: float = 0.5


class DoubleBollingerV1:
    """Frozen RSI + MACD + Double Bollinger core for SOXL/SOXS.

    Indicators: RSI(14), MACD(12,26,9), BB(20,+/-0.5 sigma), BB(20,+/-3 sigma), volume.
    CCI is intentionally NOT used.
    5m defines setup/direction; 1m is only a fresh timing check.
    Finder/ranking/other symbols are intentionally absent.
    """
    ...
    def _resample_5m_completed(self, bars_1m: pd.DataFrame) -> pd.DataFrame:
        ...
        if dt.isna().all():
            # Existing DB often stores YYYYmmddHHMMSS.
        ...
        # Use only completed 5m buckets to prevent look-ahead / half-built candle decisions.
        complete = x[x["_dt"] < current_bucket].set_index("_dt")
        if complete.empty:
            return pd.DataFrame()
        agg = complete.resample("5min").agg({
    ...
    def _five_min_setup(self, b5: pd.DataFrame):
        ...
        bb = self._bands(c)
        if len(c) < max(self.cfg.bb_period + 2, self.cfg.macd_slow + 2):
            return False, 0.0, {"reason": "INSUFFICIENT_5M"}
        ...
        # Strength is descriptive, not an extra mandatory gate.
        ...
        score = 0.0
        score += 1.0 if rsi_event else 0.0
        score += 1.0 if rsi_slope > 0 else 0.0
        score += 1.0 if macd_cross_now or macd_bull else 0.0
        score += 1.0 if hist_accel > 0 else 0.0
        score += 1.0 if inner_confirm else 0.0
        score += 1.0 if volume_ratio >= self.cfg.strong_volume_ratio else 0.0
        ...
        diag = {
            ...
            "inner_upper": float(bb["inner_upper"].iloc[-1]),
            "outer_upper": float(bb["outer_upper"].iloc[-1]),
            "volume_ratio": volume_ratio,
            "break_found": break_found,
            "above_inner": above_inner,
        }
        return setup, score, diag
    ...
    def _one_min_timing(self, bars_1m: pd.DataFrame):
        ...
        hist_up = bool(hist.iloc[-1] >= hist.iloc[-2])
        macd_ok = bool(macd.iloc[-1] > sig.iloc[-1] or (macd.iloc[-2] <= sig.iloc[-2] and macd.iloc[-1] > sig.iloc[-1]))
        rsi70_exit = bool(pd.notna(rsi.iloc[-2]) and pd.notna(rsi.iloc[-1]) and rsi.iloc[-2] >= 70 > rsi.iloc[-1])
        ok = bool((rsi_up or hist_up) and macd_ok and not rsi70_exit)
        return ok, {
            "rsi1": float(rsi.iloc[-1]) if pd.notna(rsi.iloc[-1]) else None,
            "rsi1_up": rsi_up,
            "macd1": float(macd.iloc[-1]),
            "signal1": float(sig.iloc[-1]),
            "hist1": float(hist.iloc[-1]),
            "hist1_up": hist_up,
            "rsi70_exit": rsi70_exit,
        }
    ...
    def evaluate_flat(self, symbol: str, bars_1m: pd.DataFrame) -> SignalResult:
        ...
        if bars_1m.empty:
            return SignalResult(symbol, Action.HOLD, "NO_BARS", 0.0)

        b5 = self._resample_5m_completed(bars_1m)
        setup, score, d5 = self._five_min_setup(b5)
        price = float(pd.to_numeric(bars_1m["close"], errors="coerce").iloc[-1])
        if not setup:
            return SignalResult(symbol, Action.HOLD, "WATCH_5M_SETUP", price, score=score, diagnostics={"five": d5})

        timing, d1 = self._one_min_timing(bars_1m)
        if not timing:
            return SignalResult(symbol, Action.HOLD, "WAIT_1M_TIMING", price, score=score, diagnostics={"five": d5, "one": d1})

        stop = self._entry_stop(price, bars_1m.iloc[:-1])
        if not (0 < stop < price):
            return SignalResult(symbol, Action.HOLD, "INVALID_STOP_BLOCK", price, score=score, diagnostics={"five": d5, "one": d1})

        return SignalResult(
            symbol,
            Action.ENTER,
            "RSI_MACD_INNER_BAND_CONFIRM",
            price,
            score=score,
            stop=stop,
            diagnostics={"five": d5, "one": d1, "risk_pct": (price - stop) / price},
        )
    ...
    def evaluate_open(self, state: PositionState, bars_1m: pd.DataFrame) -> SignalResult:
        if state.entry_price is None or state.stop_price is None or state.phase not in {PositionPhase.OPEN, PositionPhase.RUNNER}:
            raise ValueError("open evaluation requires valid state")
        price = float(pd.to_numeric(bars_1m["close"], errors="coerce").iloc[-1])
        ...
        b5 = self._resample_5m_completed(bars_1m)
        if len(b5) < max(self.cfg.bb_period + 2, self.cfg.macd_slow + 2):
        ...
        macd, sig, hist = self._macd(c)
        bb = self._bands(c)
        close5 = float(c.iloc[-1])
        inner = float(bb["inner_upper"].iloc[-1])
        outer = float(bb["outer_upper"].iloc[-1])
        outer_expansion = close5 >= outer
        inner_hold = close5 >= inner
        hist_up = bool(hist.iloc[-1] >= hist.iloc[-2])
        rsi_up = bool(pd.notna(rsi.iloc[-1]) and pd.notna(rsi.iloc[-2]) and rsi.iloc[-1] >= rsi.iloc[-2])
        macd_bull = bool(macd.iloc[-1] >= sig.iloc[-1])
        strength_points = int(inner_hold) + int(hist_up) + int(macd_bull)
        trend = "STRONG" if outer_expansion or strength_points >= 3 else ("NORMAL" if strength_points >= 2 else "WEAK")
        ...
        diag = {
            ...
            "rsi5": float(rsi.iloc[-1]) if pd.notna(rsi.iloc[-1]) else None,
            "rsi_up": rsi_up,
            "hist5": float(hist.iloc[-1]),
            "hist_up": hist_up,
            "macd_bull": macd_bull,
        }

        # Core trend exit: a confirmed loss of inner upper band WITH weak momentum.
        if not inner_hold and (not macd_bull or not hist_up):
            return SignalResult(state.symbol, Action.FULL_EXIT, "INNER_BAND_TREND_BREAK", price, stop=state.stop_price, exit_fraction=1.0, diagnostics=diag)

        # Optional profit decision policy. It is disabled by default so the core remains pure trend-following.
        if self.cfg.profit_check_pct is not None and not state.partial_exit_done:
            pnl = price / state.entry_price - 1.0
            if pnl >= self.cfg.profit_check_pct:
                if trend == "STRONG":
                    return SignalResult(state.symbol, Action.HOLD, "PROFIT_CHECK_STRONG_HOLD", price, stop=state.stop_price, diagnostics=diag)
                if trend == "NORMAL":
                    return SignalResult(state.symbol, Action.PARTIAL_EXIT, "PROFIT_CHECK_NORMAL_PARTIAL", price, stop=state.stop_price, exit_fraction=self.cfg.normal_partial_fraction, diagnostics=diag)
                return SignalResult(state.symbol, Action.FULL_EXIT, "PROFIT_CHECK_WEAK_EXIT", price, stop=state.stop_price, exit_fraction=1.0, diagnostics=diag)

        return SignalResult(state.symbol, Action.HOLD, "FOLLOW_DOUBLE_BOLLINGER_TREND", price, stop=state.stop_price, diagnostics=diag)
