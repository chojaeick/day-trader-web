...
def ticks_to_bars(ticks: list[dict], minutes: int=1) -> pd.DataFrame:
    ...
    if df['cum_volume'].max() > 0:
        ...
        # First bar after app restart may otherwise lose current volume; qty is preferred when available.
...
def _score_row(q:dict,m:dict,index_strength:float,semi_strength:float,progress:float):
    ...
    # User's core rule: current price above 5-day average and preferably rising MA5.
    ...
    if dollar>=500_000_000: add('Liquidity',15)
    elif dollar>=150_000_000: add('Liquidity',12)
    elif dollar>=50_000_000: add('Liquidity',8)
    elif dollar>=20_000_000: add('Liquidity',3)

    # Relative volume tiers; current cumulative volume is time-adjusted.
    ...
    # Day-trading sweet spot: enough movement but not pathological.
    ...
    # Momentum. Screen is primarily for instruments moving up today.
    ...
    elif day>15: add('Momentum',-6); penalties.append('과도한 급등')
    else: add('Momentum',-8); penalties.append('당일 음봉')
    ...
    # Long candidate means buying the instrument itself. Inverse ETF can be LONG while underlying view is bearish.
    bias='LONG' if day>=0 and price>=ma5 else ('SHORT' if day<0 and slope<0 else 'NEUTRAL')
    eligible=(price>=5 and dollar>=20_000_000 and ma5>0 and price>ma5)
    return {'symbol':sym,'score':score,'bias':bias,'price':price,'change_pct':day,'volume':vol,'dollar_volume':dollar,
            ...
            'avg5_dollar_volume':avg5dv,'eligible':eligible,'parts':parts,'penalties':penalties}
...
def context_for(symbol:str, quotes:list[dict]):
    ...
    # Normalize roughly ±4% to ±1. Inverse ETFs get the underlying bias inverted for their own long direction.
...
def multi_timeframe_signal(symbol: str, ticks: list[dict], quotes:list[dict]):
    ...
    # latest indicator snapshot for UI
