...
OPEN_PROTECT_END=10*60
HWM_DD=0.01
TIGHT_MINUTES=10


def filt_open(ev):
    return {ts:rows for ts,rows in ev.items() if pd.Timestamp(ts).hour*60+pd.Timestamp(ts).minute>=OPEN_BUY_MINUTE}


def simulate_intended(packed_exits, entry_events, state_events, threshold):
    ...
    for ts,minute,rows in packed_exits:
        ...
        for sym in list(positions):
            ...
            breakout_tight=pos['breakout_entry'] and elapsed<TIGHT_MINUTES
            opening_protect=pos['entry_minute']<OPEN_PROTECT_END and minute<OPEN_PROTECT_END
            ...
            elif opening_protect and low<=pos['completed_hwm']*(1.0-HWM_DD):
                close(sym,pos['completed_hwm']*(1.0-HWM_DD),ts,'OPENING_HWM_1PCT_EXIT')
            elif breakout_tight and low<=pos['completed_hwm']*(1.0-HWM_DD):
                close(sym,pos['completed_hwm']*(1.0-HWM_DD),ts,'BREAKOUT_FIRST10_HWM_1PCT_EXIT')
            elif opening_protect or breakout_tight:
        ...
        if minute<base.NO_ENTRY_MINUTE:
            for c in entry_events.get(ts,[]):
                ...
                positions[sym]={'symbol':sym,'entry_time':pd.Timestamp(ts),'entry_minute':minute,'entry_price':float(closep),'stop_price':float(closep)-float(stop_dist),'tp1_price':float(closep)+2.0*float(band_r),'remaining':1.0,'realized':0.0,'tp1_done':False,'tp2_done':False,'tp1_bar_high':np.nan,'post_tp1_high':-np.inf,'fade_armed':False,'fast_fade_streak':0,'breakout_entry':bool(breakout),'completed_hwm':float(closep)}; last_price[sym]=float(closep)
...
def main():
    ...
    # No inherited V15/V16 entry WAIT. 09:00-09:09 is the only buy blackout.
    empty_waits=pd.DataFrame()
    ev,added,skipped=v17b.build_v17b(ev10,scored,empty_waits)
    t=simulate_intended(packed,ev,states,THRESHOLD)
    print('=== V17C MULTI — INTENDED OPENING RULE ===')
    print('BUY: 09:00-09:09 blocked only; 09:10+ valid signals tradable immediately.')
    print('RISK: positions entered 09:10-09:59 use completed-HWM -1% full exit until 10:00; then ordinary V17C exits. Breakout first-10m HWM remains.')
    multi.metrics('INTENDED_OPENING_RULE',t)
    ...
    print('\nOPENING_HWM_EXITS=',int((t.reason=='OPENING_HWM_1PCT_EXIT').sum()))
    print('BREAKOUT_ADDED=',added); print('BREAKOUT_SKIPPED=',skipped)
    out='/home/ubuntu/day-trader-api/engine5_v16_full_validation/v17c_multi_intended_opening_rule.csv'; t.to_csv(out,index=False); print('[CSV]',out)
