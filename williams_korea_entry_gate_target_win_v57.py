...
def entry_feats(c,h,l,v,idx):
    ...
    return dict(
        ...
        r14=r14[i] or 50, r14s1=slope(r14,i,1), r14s3=slope(r14,i,3),
        cci=cc[i] or 0, ccis1=slope(cc,i,1), ccis3=slope(cc,i,3),
        mac_above=mac[i]>sig[i], macs1=slope(mac,i,1), macs3=slope(mac,i,3),
...
def main():
    ...
    for s in syms:
        ...
        for di in range(1,len(ds)):
            ...
            f=entry_feats(c,h,l,v,idx); ret=run_exit(cur,idx+1)
            mfe=max(pct(c[idx+1],x) for x in h[idx+1:])
            rows.append(dict(d=d,s=s,ret=ret,mfe=mfe,**f))
    dates=sorted(set(x['d'] for x in rows)); cut=len(dates)//2; isd=set(dates[:cut]); oosd=set(dates[cut:])
    tests={
      'MAC_ABOVE':lambda x:x['mac_above'],
      'H3>0':lambda x:x['hists3']>0,
      'H1>0':lambda x:x['hists1']>0,
    ...
    print('=== WILLIAMS KOREA ENTRY GATE TARGET WIN V57 ===')
    print('Search simple causal entry gates. Rank by OOS win rate, then PF. Diagnostic only.')
    ...
    for k in (1,2,3):
        for combo in itertools.combinations(names,k):
            ...
            iset=[x for x in rows if x['d'] in isd and fn(x)]
            oset=[x for x in rows if x['d'] in oosd and fn(x)]
            if len(iset)<4 or len(oset)<4: continue
            mi,mo=met(iset),met(oset)
            candidates.append((mo[1],mo[3],mo[0],mo[2],mi[1],mi[3],combo))
    candidates.sort(key=lambda z:(z[0],z[1],z[2]),reverse=True)
    print('--- TOP OOS WIN GATES ---')
    for z in candidates[:30]:
        print(f"{'+'.join(z[6])} OOS_N={z[2]} WIN={z[0]:.1f}% AVG={z[3]:.3f}% PF={z[1]:.3f} | IS_WIN={z[4]:.1f}% IS_PF={z[5]:.3f}")
    print('--- BASE ---')
    print('ALL',met(rows))
    con.close()
