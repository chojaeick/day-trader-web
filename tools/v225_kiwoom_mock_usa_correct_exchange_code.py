...
SYMBOL='AAPL'
STEX_TP='ND'  # Kiwoom US: ND NASDAQ, NY NYSE, NA AMEX
QTY=1

print('=== V225 KIWOOM USA MOCK CORRECT EXCHANGE/LIMIT CODES ===')
print('TARGET=KIWOOM_MOCK_ACCOUNT USA REAL_ACCOUNT_ALLOWED=NO')
print('SYMBOL=',SYMBOL,'STEX_TP=',STEX_TP,'TRDE_TP=00 LIMIT_ONLY')
...
if base!='https://mockapi.kiwoom.com': raise SystemExit('V225_ABORT non-mock base')
if os.environ.get('KIWOOM_MOCK_ORDER_ENABLE','0').lower() not in ('1','true','yes','on'): raise SystemExit('V225_ABORT mock order disabled')
...
if not token: raise SystemExit('V225_ABORT token failed')
print('TOKEN_OK=True')
...
def post(path,api_id,body):
    ...
    print('HTTP',api_id,rr.status_code); rr.raise_for_status(); jj=rr.json(); print(api_id,'BODY=',json.dumps(jj,ensure_ascii=False)); return jj
...
if px<=0: raise SystemExit('V225_ABORT no AAPL price')
pre=balance(); preqty=holding_qty(pre); print('PRE_QTY=',preqty)
if preqty>0: raise SystemExit('V225_ABORT existing AAPL holding')
...
if not accepted: raise SystemExit('V225_ABORT buy not accepted')
...
if not buy_confirm: raise SystemExit('V225_ABORT buy fill not confirmed')
...
if not sell_acc: raise SystemExit('V225_ABORT sell not accepted')
...
print('V225_KIWOOM_USA_MOCK_SMOKE_PASS=', bool(accepted and buy_confirm and sell_acc and closed))
